...
class SerialGUI:
    ...
    def create_widgets(self):
        ...
        ttk.Button(frame, text="Refresh Ports", command=self.update_ports).grid(
            row=0, column=2, sticky=tk.W
        )

        ttk.Label(frame, text="Axis:").grid(row=1, column=0, sticky=tk.W)
        ttk.Entry(frame, textvariable=self.axis_var).grid(
        ...
        ttk.Label(frame, text="Parameters:").grid(row=3, column=0, sticky=tk.W)
        ttk.Entry(frame, textvariable=self.params_var).grid(
            row=3, column=1, sticky=(tk.W, tk.E)
        )

        ttk.Button(frame, text="Send Command", command=self.send_command).grid(
            row=4, column=1, sticky=tk.E
        )

        ttk.Label(frame, text="Result:").grid(row=5, column=0, sticky=tk.W)
        self.result_label = ttk.Label(frame, textvariable=self.result_var)
        self.result_label.grid(row=5, column=1, sticky=(tk.W, tk.E))

        self.history_text = tk.Text(frame, height=10, wrap=tk.WORD)
        self.history_text.grid(row=6, column=0, columnspan=3, sticky=(tk.W, tk.E))

        self.clear_button = ttk.Button(frame, text="Clear", command=self.clear_result)
        self.clear_button.grid(row=7, column=0, sticky=tk.W)

        self.poll_var = tk.BooleanVar()
        self.poll_checkbox = ttk.Checkbutton(
            frame, text="Enable Continuous Polling", variable=self.poll_var
        )
        self.poll_checkbox.grid(row=7, column=1, sticky=tk.W)

        self.poll_button = ttk.Button(
            frame, text="Start Polling", command=self.toggle_polling
        )
        self.poll_button.grid(row=7, column=2, sticky=tk.E)
    ...
    def update_ports(self):
        ...
        port_list = [
            port.device
            for port in ports
            if port.manufacturer is not None and "FTDI" in port.manufacturer
        ]
        ...
        try:
            with MicronixSerialConnection(port) as conn:
                if params:
                    ...
                    result = conn.execute_command(axis, command, *params_list)
                else:
                ...
                if result:
                    ...
                    self.history_text.insert(
                        tk.END,
                        f"Sent: {axis}{command.command} {params}\nReceived: {result}\n\n",
                    )
                else:
